"""Programa para Encriptar/Desencriptar por el método Alberti con Índice de Letras"""
...
def encriptar(texto,A,g):
    ...
    for char in texto:
        ...
        if char in disco_afuera_upper and not is_int(char):
            idx_disco_afuera = disco_afuera_copia.index(char)
...
def desencriptar(texto,A,g):
    ...
    for char in texto:
        ...
        if char in disco_adentro_copia:
            if not is_int(disco_afuera_copia[disco_adentro_copia.index(char)]):
                texto_encriptado += disco_afuera_copia[disco_adentro_copia.index(char)]
            continue

        elif char not in disco_afuera_lower and char not in disco_afuera_upper:
            texto_encriptado += char

    print(f"Texto desencriptado: {texto_encriptado}")

def albertiletras():
    #Se ponen entre tres comillas lo que queremos que diga que hace la función
    """Es la función principal del programa Alberti con Índice de Letras"""
